...
def create_bash_scripts(cpu, replica, client):
    for r in range(replica):
        ...
        lines.append("#SBATCH --reservation=cpu-s2-moka_blox-0_31"+"\n")
        ...
        lines.append("srun --nodelist=cpu-" + str(cpu[r]) + " sleep 2" + "\n")
        lines.append("srun --exclusive --ntasks=1 --cpus-per-task=16 --nodelist=cpu-" + str(cpu[r]) + " singularity exec resilient ./rundb -nid" + str(r)+"\n")
        PATH = './s' + str(r) + '.sh'
        f = open(PATH,'w')
        f.writelines(lines)
        f.close()
    ...
    for c in range(replica,replica + client):
        ...
        lines.append("sleep 5"+"\n")
        #lines.append("sbcast -f config.h config.h"+"\n")
        lines.append("srun --nodelist=cpu-" + str(cpu[c]) + " sleep 2" + "\n")
        lines.append("srun --exclusive --ntasks=1 --cpus-per-task=8 --nodelist=cpu-" + str(cpu[c]) + " singularity exec resilient ./runcl -nid" + str(c)+"\n")
...
if(len(sys.argv) == 3):
    print("Warning: rundb & runcl must be compiled with correct values in config.h")
